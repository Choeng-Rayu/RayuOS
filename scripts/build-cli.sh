#!/bin/bash
# RayuOS CLI Edition Build Script
# Builds a minimal CLI-only ISO using live-build
...
lb config \
    ...
    --memtest none \
    --iso-application "RayuOS CLI" \
    --iso-publisher "RayuOS Project" \
    --iso-volume "RAYUOS_CLI" \
...
# Copy CLI-specific configuration
echo "Applying CLI edition configuration..."
cp "$PROJECT_ROOT/configs/cli/packages.list.chroot" config/package-lists/
mkdir -p config/hooks/live
cp "$PROJECT_ROOT/configs/cli/hooks/live/"* config/hooks/live/ 2>/dev/null || true
...
if [ -n "$ISO_FILE" ]; then
    ...
    echo ""
    echo "Test with: ./scripts/test-qemu.sh $ARTIFACTS_DIR/$OUTPUT_NAME"
else
